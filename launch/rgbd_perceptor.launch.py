...
def get_nvblox_params(num_cameras: int, nvblox_config: dict, common_config: dict) -> List[Any]:
    ...
    for config_file in nvblox_config.get('config_files', []):
        ...
        parameters.append(lu.get_path(
            config_file.get('package', 'isaac_ros_perceptor_realsense_d456'),
...
def start_cuvslam(args: lu.ArgumentContainer, cuvslam_config: dict, common_config: dict) -> Action:
    ...
    # Add the imu topic if provided (enable_imu_fusion controlled via parameters section)
    imu_config = cuvslam_config.get('remappings', {}).get('imu')
    if imu_config is not None:
        imu_topic = imu_config if isinstance(imu_config, str) else imu_config.get('topic')
        if imu_topic:
...
def generate_launch_description_impl(args: lu.ArgumentContainer) -> List[Action]:
    config_file = lu.get_path('isaac_ros_perceptor_realsense_d456', args.config_file)
